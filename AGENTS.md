...
.claude/skills/      the skills loaded at the trigger points below
...
- **pr-workflow** — after approval, on the PR path (the two-pass gate pipeline,
  branch naming, PR-description style, and the merge gate). Scale rigor per
  task with the `fast` / `standard` / `default` / `rigorous` tiers — the skill
...
defaults, `docs/`) is the shared distro; `state/`, `repos/`, `data/`, `.env`
are operator-private and gitignored. Ship changes to the tracked surface
through this repo's own PR path.
...
- `bin/` must run on bash 3.2 (macOS default) — no `mapfile`, no `declare -A`,
  ...
  constructs, and a `bash:3.2` container runs the whole suite — harness AND
  toolbelt — on real 3.2 (`.dm-knowledge/ci.md`).
...
- `.dm-knowledge/ci.md` — the CI job graph, the fail-safe path filter, the
