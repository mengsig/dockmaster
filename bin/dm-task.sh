...
case "$cmd" in
  ...
  set)
    ...
    case "$key" in
      ...
      # The tests gate's result, the command that produced it, and the code-state
      # fingerprint (#185) it was recorded against. All three are published on
      # the PR as evidence (#175), so a hand-set value would put a pass a
      # reviewer trusts behind a suite that never ran, or unpin it from the code
      # it actually tested. dm-test.sh writes them through dm_meta_set directly.
      tests|tests_cmd|tests_head) dm_die "'$key' is recorded by dm-test.sh when the tests gate runs; hand-setting it would publish a test result nothing ran, or unbind it from the code it tested" ;;
