...
- Codex local memory — optional runtime recall only; never the sole owner of a
  required fact. The checked-in and `state/` stores above remain authoritative.
...
| the operator (prefs, style) | `state/operator.md` + optional Codex local memory |
