...
thread_name="$(bin/dm-thread-name.sh <child-id> worker)"
bin/dm-task.sh set <child-id> thread_name "$thread_name"
spawn_agent(task_name=<thread_name>, message=<brief>, fork_turns="none")
...
interrupt that exact returned id and leave/requeue it visibly. Nothing else is
